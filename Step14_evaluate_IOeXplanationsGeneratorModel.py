######### Step14_evaluate_IOeXplanationsGeneratorModel.py
...
###### Load the best eXplanationGenerator model obtained after running 'Step13_train_IOeXplanationsGeneratorModel.py'
# Xmodel_name = 'saved_models/perScenarioModel_wContextConcat_IB_interim128_wextras-e11-best_model.ckpt'
Xmodel_name = 'models/tsn_flow_TOIAlignedFeats_wContext_IDDeXplanationGenerator.ckpt'

interim_layerdim = 128
if('Context' in Xmodel_name):
...
model.load_state_dict(torch.load(Xmodel_name, map_location=device)['state_dict'])
model = model.to(device)
print("#################################### This is the MODEL NAME:",Xmodel_name)
...
# all_batch_val_feats = np.load('all_batch_feats_iddx_motwise_val.npy',allow_pickle=True)
# all_batch_val_labels = np.load('all_batch_labels_iddx_motwise_val.npy',allow_pickle=True)
# output_valfeats_path =  'all_batch_feats_iddx_motwise_wContextfeats_val.npy'
# output_valfeats_path =  'all_batch_feats_iddx_motwise_wContextfeats_val_wextras.npy'
output_valfeats_path =  'all_batch_feats_iddx_motwise_wContextfeats_val_wextras_new_rectifiedXlabels.npy'
if(not os.path.exists(output_valfeats_path)):
    all_batch_val_feats = None
    all_batch_val_labels = []
else:
    all_batch_val_feats = np.load(output_valfeats_path, allow_pickle=True)
    all_batch_val_labels = np.load(output_valfeats_path.replace('all_batch_feats', 'all_batch_labels'), allow_pickle=True)
    ...
    for phase in ['val']:
        ...
        if phase == 'train':
            ...
            if(not os.path.exists(output_valfeats_path)):
                ...
                all_batch_val_labels = np.array(all_batch_val_labels)
                ### batching saved feats and labels
                epoch_batches = []
                for batch_idx in range(0, len(all_batch_val_labels), batch_samples_size):
                    epoch_batches.append(list(range(batch_idx, min(batch_idx+batch_samples_size, len(all_batch_val_labels)))))
        if(len(epoch_batches[-1])<=1):
            print(len(epoch_batches[-1]), epoch_batches[-1])
        ...
        all_gt_labels = None
        for batch_idx, ebi in enumerate(epoch_batches):
            if(phase=='val' and os.path.exists(output_valfeats_path)):
                if('Context' in Xmodel_name):
                    batch_feats = all_batch_val_feats[ebi,:]
                else:
                    batch_feats = all_batch_val_feats[ebi,:2048]
                batch_labels = all_batch_val_labels[ebi]
                n_examples+= len(batch_labels)
            else:
            ...
            #     labels_batch = all_labels[batch_idx:batch_idx+batch_samples_size]
                ...
                for video_dir, track_bboxes, labels_ in zip(video_dirs_batch, track_bboxes_batch, labels_batch):
                    if(phase=='train'):
                        ...
                        # ### Possibly a very hard augmentation, it is maybe cropping out the original signal which maybe located in a very small temporal duration somewhere midway
            ...
            if(perClip):
                ...
                ### check if incorrect impl above (shuffling should be done at dim=1 

            if(not os.path.exists(output_valfeats_path)):
                ### saving in cold storage
                if(all_batch_val_feats is None):
                    all_batch_val_feats = batch_feats.copy()
                else:
                    all_batch_val_feats = np.vstack((all_batch_val_feats, batch_feats.copy()))
                all_batch_val_labels.extend(batch_labels)
            ...
            if(all_pred_labels is None):
                all_pred_labels = pred_exp1.cpu()
            else:
                all_pred_labels = torch.cat([all_pred_labels, pred_exp1.cpu()])
            if(all_gt_labels is None):
                all_gt_labels = batch_labels.cpu()
            else:
                all_gt_labels = torch.cat([all_gt_labels, batch_labels.cpu()])
            ...
            # break
            # del loss, output, _, pred_exp1, batch_feats, batch_labels
            if(not os.path.exists(output_valfeats_path)):
                del roi_TAF, roi_F, roi_CF
            del batch_feats, batch_labels, loss, _, pred_exp1, output
        ...
        performance_out = classification_report(all_gt_labels, all_pred_labels, target_names= labels_order.tolist(), zero_division=1, digits=3)
        
    if(not os.path.exists(output_valfeats_path)):
        np.save(output_valfeats_path, all_batch_val_feats)
        np.save(output_valfeats_path.replace('all_batch_feats', 'all_batch_labels'), all_batch_val_labels)

print(performance_out)
# disp = ConfusionMatrixDisplay(confusion_matrix= confusion_matrix(all_gt_labels, all_pred_labels))
# disp.plot()
