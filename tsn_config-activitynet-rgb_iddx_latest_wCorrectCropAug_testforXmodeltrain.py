    ...
    test=dict(
        ...
        pipeline=[
            dict(
                ...
                frame_interval=1,
                num_clips=8,
                test_mode=False),  ### for randomized sampling of frames
                # test_mode=True),
            dict(type='RawFrameDecode'),
            # dict(type='Resize', scale=(-1, 256)),
            # dict(type='CenterCrop', crop_size=(420, 208)),
            # dict(type='Resize', scale=(224, 224), keep_ratio=False),
            dict(type='Resize', scale=(-1, 224)),   ### retaining the full image (to avoid cropping context near the periphery)
            dict(type='Flip', flip_ratio=0.5),   ### for augmenting while training X model
            dict(
                type='Normalize',
                mean=[128, 128],
                std=[128, 128],
                to_bgr=False),
            dict(type='FormatShape', input_format='NCHW_Flow'),
            dict(type='Collect', keys=['imgs', 'label'], meta_keys=['frame_inds', 'original_shape', 'img_shape', 'keep_ratio', 'scale_factor', 'flip']),  ##, 'crop_bbox'
            dict(type='ToTensor', keys=['imgs'])
        ],
        num_classes=3))
evaluation = dict(
